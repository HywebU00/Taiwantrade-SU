$(function(){
  ...
  _body.append('<div class="coverAll"></div><div class="sidebarMask"></div>');
  var _coverAll = $('.coverAll');
  ...
  */
  ...
  _goBottom.click(function(){
    _html.stop(true,false).animate({scrollTop: _body[0].scrollHeight}, 1200);
  });



  // 模擬 select 下拉選單 ------------------------------------------
  var _selectImitatingsss =   $('.selectImitating');
  _selectImitatingsss.each(function(){
    let _selectImitating = $(this);
    let _selectBtn = _selectImitating.find('.selectBtn');
    let _optionGroup = _selectImitating.find('.optionGroup');
    // let _option = _optionGroup.find('input[type="checkbox"]');
    const speed = 250;
    ...
    _optionGroup.css({
      ...
      'top' : _selectBtn.outerHeight(true) // 202307修改
    })

    // 模擬 placeholder 文字
    let placeholder = '<span class="placeholder">' + _selectBtn.attr('data-title') + '</span>';
    if (_selectBtn.text() == "") {
    ...
    _selectBtn.click(function(){
      let optMaxHeight = window.innerHeight - _selectImitating.offset().top + $(window).scrollTop() - 60;// 20210414
    ...
    _selectBtn.each(function (){
      ...
      _checkOption.click(function(){
        ...
        if(_this.parent().hasClass('checked')){
          ...
          let a = selected.indexOf(_this.parent().text());
          selected.splice(a, 1);
      ...
      // 單選的下拉選單
      let _option = $(this).next('.optionGroup').find('.selectOption>li');
  ...
  _popMsg.each(function(){
    ...
    _closePop.click(function(){
      _this.fadeOut(200, function(){
        _this.removeAttr('style').prev('.coverAllNew').remove();
      });

      // 燈箱內開啟的燈箱
      if (_this.parents('.lightbox').length > 0) {
        _this.prev('.coverAll').remove();
      } else {
        _coverAll.fadeOut(200);
  ...
  $('.trigPop').click(function(){
    let _this = $(this);
    try {
      var _openWhich = _popMsg.filter('#'+ _this.attr('id').slice(4));
      _openWhich.fadeIn(200);

      // 如果 $('.trigPop') 是在燈箱內
      if ( _this.parents('.lightbox').length > 0) {
        _coverAll.clone().insertBefore(_openWhich).show().css('z-index', 9998).addClass('coverAllNew');
        _openWhich.css('z-index', 9999);
      } else {
  ...
  _showReact.click(function(){
    ...
    _html.stop(true,false).animate({scrollTop:_inThisTr.first().offset().top - hh}, 600);
  })
  ...
  var _multiCategory = $('.multiCategory');
	_multiCategory.each(function(){
    let _this = $(this);

		let _cateSelect = _this.find('.cateSelect');
		let _cateList = _cateSelect.find('.cateList').not('.bySearch');
		let _cateList1 = _cateList.find('ul').first();
		let _cateList2 = _cateList.find('ul').eq(1);
		let _cateList3 = _cateList.find('ul').eq(2);
    ...
    // let _pdCateSel = _pdCateInput.next('button');
      ...
      let _this = $(this);
			if (_cateSelect.is(':visible')) {
				_cateSelect.slideUp();
				_this.removeClass('cancel').text(btnText1);
				_pdCateOk.hide();
			} else {
          ...
          _cateList2.show();
				...
				_this.addClass('cancel').text(btnText2);
				_pdCateOk.show();
			}
    });

    _pdCateInput.click(function(){
      _cateSelect.slideDown();
...
// Product Requested 下拉選單模擬
  ...
  var _tabs = $('.tabGroup');
  _tabs.each(function(){
    let _thisTabGroup = $(this);
    let _tabItem =_thisTabGroup.find('.tabs').find('li');
    let _tabContainer =_thisTabGroup.find('.tabContainer');

    _tabContainer.eq(_tabItem.filter('.active').index()).addClass('show');
    
    if(! _thisTabGroup.hasClass('disabled')) {
  ...
  lightbox();
    ...
    var _hideLightbox = _lightbox.find('.hideLightbox');
	  ...
	  if(_showLightbox.length>0) {
	      _showLightbox.click(function(){
	      var lbxid = '.' + $(this).attr('id');
				// _lightbox.filter(lbxid).show();
	      let _lightboxNow = _lightbox.filter(lbxid).show();
	      let _coverAllNow = _coverAll.clone().insertBefore(_lightboxNow).show();
	      let _hideLightbox = _lightboxNow.find('.hideLightbox');
	
	      _body.addClass('noScroll');
	
	      _coverAllNow.click(function(){
	        $(this).add(_lightboxNow).hide();
	        $(this).remove();
	        if( _lightbox.filter(':visible').length == 0 ) {
	          _body.removeClass('noScroll');
	        }
  ...
  // 20230817 
  var _closeNationsOpt = _nationsOpt.find('.hideLightbox');
  _closeNationsOpt.click(function(){
    $(this).parents('.lightbox').scrollTop(0);
  })
  ...
  && _threads.children('ul').children('li').not('.marker').length > 1 
    ...
    _html.stop(true,false).animate({scrollTop: unReadMarkerOffsetTop}, 800, function(){
      if( _companyNow.hasClass('fixed')){
        _html.stop(true,false).animate({scrollTop: unReadMarkerOffsetTop - _companyNow.innerHeight()}, 600)
      }
    });
  }
...
// 表格展開／收合 20200630
  ...
  // 廣告活動報表的「狀態」欄位說明文字
  // ------------------------------------------------
  var _adStatusNow = $('.listTbNew').find('.adStatusInfo').prev('.adStatusNow');
  _adStatusNow.addClass('hasInfo');
  var _adiOffset;
  _adStatusNow.on('mouseenter', function(){
    let _this = $(this);
    _adiOffset = _this.offset();
    _this.next().fadeIn(200).offset({
      top: _adiOffset.top - 4,
      left: _adiOffset.left + _this.innerWidth() + 7
    });
...
// ------------------------------------------------
  ...
  // [廣告報表設定]設定編輯區收合效果 ////////////////////////////////////////
  var _adSettingField =  $('.adSettingThis').find('.fieldSet');
  _adSettingField.each(function(){
    let _this = $(this);
    let _changeSetting = _this.find('.changeSetting');
    let _controler = _changeSetting.prevAll('.legend');
    let _settingNow = _changeSetting.prevAll('.settingNow');
    const speed = 300;

    _this.find('.legend').not(_controler).addClass('readOnly');

    if ( _this.hasClass('reveal') ) {
      _settingNow.hide();
      _changeSetting.show();
    }

    _controler.add(_settingNow).on('click', function(){
      if ( _this.hasClass('reveal') ) {
        _changeSetting.slideUp(speed, function(){
          _settingNow.fadeIn(speed);
          _this.removeClass('reveal');
        });
      } else {
        _settingNow.hide();
        _changeSetting.slideDown(speed);
  ...
  // 出價：不同 select option 切換設定內容 ///////////////////////////////////
  var _bidSetting = $('.bidSetting');
  _bidSetting.each(function(){
    let _this  = $(this);
    let _select = _this.find('select');
    let _option = _select.find('option');
    let _eachOptionSetting = _this.find('.eachOptionSetting');

    _eachOptionSetting.eq(_option.filter(':selected').index()).show();
    _select.on('change', function(){
      _eachOptionSetting.hide().eq(_option.filter(':selected').index()).show();
    })
  })


  // 改變廣告狀態 ////////////////////////////////////////
  var _adStatusSelect = $('.adStatusSelect');
  var _adStatusOption = _adStatusSelect.find('li');
  var _closeBtn = _adStatusSelect.find('button');
  var _adStatusClass;

  var _adTitleGroup = $('.adTitleGroup');
  var _adStatusBtn = _adTitleGroup.find('.adStatus');
  var _adStatusBtnNow;

  _adStatusBtn.click( function () {
    _adStatusBtnNow = $(this);
    if (! _adStatusBtnNow.hasClass('removed')) {
      let _offset = _adStatusBtnNow.offset();
      _adStatusSelect.fadeIn(160).offset({top: _offset.top, left: _offset.left });
    }
  })
  _adStatusOption.click(function(){
    _adStatusClass = $(this).children('.adStatus').attr('class');
  })
  _closeBtn.click(function(){
    _adStatusSelect.hide(200);
  })
  $('#adStatusChanged').find('.closePop').click( function(){
    _adStatusSelect.hide().removeAttr('style');
    _adStatusBtnNow.removeClass().addClass(_adStatusClass);
  })
  $('#adRemoveConfirm').find('.confirm').click( function(){
    $('.popMsg#adStatusChanged').show(220,function(){
      _coverAll.show();
    });
  })

  $('#adRemoveConfirm').find('.closePop').not('.confirm').click(function(){
    _adStatusSelect.hide(200);
  })

  // 操作提示（廣告設定） ////////////////////////////////////////
  var _textSlide = $('.textSlide');
  _textSlide.each(function () {
    let _this = $(this);
    let _slideBox = _this.find('.slideBox');
    let _slideItem = _slideBox.find('.slideItem');
    let count = _slideItem.length;
    let _btnPrev = _this.find('.prev');
    let _btnNext = _this.find('.next');
    let n = 0;
    let m;
    const classShow = "in";
    const classHide = "fadeOut"

    _slideItem.eq(n).addClass(classShow).siblings().addClass(classHide);
    
    _btnNext.click(function(){
      _slideItem.removeClass('otherside');
      m = (n+1) % count;
      setTimeout( textSlide , 10 );
    })

    _btnPrev.click(function(){
      _slideItem.addClass('otherside');
      m = (n-1) % count;
      setTimeout( textSlide , 10 );
    })

    function textSlide() {
      _slideItem.eq(n).addClass(classHide);
      _slideItem.eq(m).removeClass(classHide).addClass(classShow);
      setTimeout(
        function(){ 
          _slideItem.eq(n).removeClass(classShow);
          n = m;
        } , 510 
      );
    }
  ...
  // 有 checkbox 的下拉選單，選到的項目可再移除 ////////////////////////////////////////
  var _slideDownChecking = $('.slideDownChecking');
  _slideDownChecking.each( function(){
    let _this = $(this);
    let _controler = _this.find('.slideCtrl');
    let _slideArea =_this.find('.slideArea');

    _controler.click(function(){
      if (_slideArea.is(':visible')) {
        _slideArea.slideUp(400, function(){
          _slideArea.removeAttr('style');
        })
        _controler.removeClass('turn');
      } else {
        _slideArea.slideDown(400, function(){
          if ( _slideArea.offset().top + _slideArea.height() >  _body.height()) {
            _slideArea.height(_body.height() - _slideArea.offset().top - 30);
          }
        })
        _controler.addClass('turn');
      }
    })
  ...
  // 廣告優異度：環 ////////////////////////////////////////
  var _adPercentageDonut = $('.adStrength').find('.percentageDonut');
  var adPercentage = parseInt( _adPercentageDonut.attr('data-percentage') )/100;
  var styleText = 'background-image:conic-gradient( var(--color) '+ adPercentage + 'turn, var(--bgColor) '+ adPercentage + 'turn);'
  _adPercentageDonut.attr('style', styleText);



  // 廣告活動最佳化分數（百分比 bar） ////////////////////////////////////////
  var _adPercentageBar = $('.adSummarize').find('.percentageBar');
  _adPercentageBar.each(function(){
    let value = $(this).find('.percentage').text();
    $(this).find('.bar').after('<style> .bar::before{width:'+ value + ';} </style>' );
  })
  ...
  // 2024新作「多選下拉」 //////////////////////////////////////////////////////////
  const _multiSelection = $('.multiSelection'); 
  _multiSelection.each( function(){
    let _this = $(this);
    let _button = _this.find('button.placeholder');
    let _options = _this.find('.options');
    let optionMaxHeight;
    const speed = 300;
    
    _button.on('click', function(){
      optionMaxHeight = window.innerHeight - _this.offset().top + $(window).scrollTop() - 60;
      if (_options.is(':visible')) {
        _options.slideUp(speed, function(){
          _options.removeAttr('style');
        });
        _button.removeClass('up');
      } else {
        _options.css('max-height', optionMaxHeight ).slideDown(speed);
        _button.addClass('up');
      }
    })
  })
  ...
  const _dateInput = _dateRangePicker.find('input.dateRange');
  const _rangePicker = _dateRangePicker.find('.pickRange');
  _dateInput.on('click', function(){
    if (_rangePicker.is(':visible')) {
      _rangePicker.hide();
    } else {
      _rangePicker.show().css('display', 'flex');
    }
  })
  ...
  // 202408 小 i 訊息 //////////////////////////////////////////////////////////
  var _iButton = $('.heading').find('.icon_i.showInfo');
  var _hiddenInfo = $('.hiddenInfo');
  const iShiftX = 80;

  _iButton.on('mouseenter', function(){
    let _this_i = $(this);
    const ih = _this_i.innerHeight() + 8;
    _hiddenInfo.css(
      {
        'left': _this_i.offset().left - iShiftX,
        'top': _this_i.offset().top + ih
      }
    ).fadeIn(300);
  })
